...
WaitForXHR.prototype.command = function (requestURL, timeout_wait_in_ms, callback_WaitForXHR, trigger) {
  ...
  self.api.waitForElementPresent("html", 5000);
  ...
  self.child_interval = setInterval(function () {
    ...
    self.api.execute(function (windowResponseContainer) {
      ...
      if (result && result.state && result.state === 'success' && result.value) {
        //console.log(util.format('XHR catched value: %s', JSON.stringify(result.value)));
        var jsonRequestData = result.value.request_data || '';
        try {
          jsonRequestData = JSON.parse(jsonRequestData);
        } catch(e) {}
        callback_WaitForXHR && callback_WaitForXHR(result.value.status, result.value.method, result.value.url, result.value.http_response_code, jsonRequestData);
        //console.log('CLEAR INTERVAL START');
        clearInterval(self.child_interval);
        clearTimeout(self.master_timeout);
  ...
  self.master_timeout = setTimeout(function () {

    // print message
    console.log(util.format('[WaitForXHR TimeOut] %s ms, missing_XHR_request for regex string: "%s"', timeout_wait_in_ms, requestURL));
